...
contract Auth {
    ...
    /// @dev the authorizer includes `commit` in their signature to ensure the authorized contract will only execute
    /// intended actions(s).
    ///      the Invoker logic MUST implement constraints on the contract execution based on information in the
    /// `commit`;
    ...
    /// @dev signature values (yParity, r, s) are interpreted as an ECDSA signature on the secp256k1 curve over
    /// getDigest(commit).
